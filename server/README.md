...
└── routes
    └── index.js
```
`app.js` 是 Server 的主入口文件，Server 使用 Koa 框架，在 `app.js` 创建一个 Koa 实例并响应请求。

`routes/index.js` 是 Server 的路由定义文件

`controllers` 存放 Server 所有业务逻辑的目录，`index.js` 不需要修改，他会动态的将 `controllers` 文件夹下的目录结构映射成 modules 的 Object，例如 Server 中的目录将会被映射成如下的结构：
